...
const getListLibs = (data) => {
  ...
  return async dispatch => {
    if (data.search) {
      await axios.get(`${url}/diseases/?page_size=10&page=${data.pageNumber}&search_text=${data.search}&sort_by=id&order=asc`, {
        headers: {
      ...
      await axios.get(`${url}/diseases/?page_size=10&page=${data.pageNumber}&sort_by=id&order=asc`, {
        headers: {
          'content-type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem("accessToken")}`
        }
